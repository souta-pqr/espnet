...
from espnet2.tasks.asr import DisfluencyASRTask  # Changed to DisfluencyASRTask
...
# Alias for typing - Extended for disfluency (4-class classification)
ListOfHypothesis = List[
    Tuple[
        ...
        Optional[List[int]],  # Added for disfluency predictions (0:fluent, 1:filler, 2:reparandum, 3:interjection)
    ]
]
...
class DisfluencySpeech2Text:
    """DisfluencySpeech2Text class for joint ASR and disfluency detection

    Examples:
        >>> import soundfile
        >>> speech2text = DisfluencySpeech2Text("asr_config.yml", "asr.pth")
        >>> audio, rate = soundfile.read("speech.wav")
        >>> speech2text(audio)
        [(text, token, token_int, hypothesis object, disfluency_predictions), ...]

    Disfluency classes:
        0: fluent (流暢)
        1: filler (フィラー: えー、あの、など)
        2: reparandum (言い直し)
        3: interjection (感動詞)
    ...
    def __init__(
        ...
        disfluency_classes: int = 4,  # 0: fluent, 1: filler, 2: reparandum, 3: interjection
    ):

        task = DisfluencyASRTask if not enh_s2t_task else EnhS2TTask
        ...
        if ctc_weight > 0.0 and hasattr(asr_model, "ctc") and asr_model.ctc is not None:
        ...
        if asr_model.use_transducer_decoder:
            ...
            if ctc_weight == 0.0:
                scorers.pop("ctc", None)
                weights.pop("ctc", None)
        ...
        # Disfluency-specific attributes
        self.disfluency_weight = disfluency_weight
        self.disfluency_classes = disfluency_classes
        self.use_disfluency_detection = getattr(asr_model, 'use_disfluency_detection', False)
    ...
    def __call__(self, speech: Union[torch.Tensor, np.ndarray]) -> Union[
        ...
        Returns:
            text, token, token_int, hyp, disfluency_predictions
    ...
    def _decode_single_sample(self, enc: torch.Tensor) -> ListOfHypothesis:
                ...
                if isinstance(self.beam_search.nn_dict.decoder, S4Decoder):
                    for module in self.beam_search.nn_dict.decoder.modules():
                        if hasattr(module, "setup_step"):
                            module.setup_step()
            # --- ここを追加 ---
            if hasattr(self.asr_model, "decode_with_disfluency"):
                nbest_hyps = self.asr_model.decode_with_disfluency(
                    encoder_out=enc.unsqueeze(0),  # (1, T, D)
                    encoder_out_lens=torch.tensor([enc.shape[0]], dtype=torch.long, device=enc.device),
        ...
        for hyp in nbest_hyps:
            # dictの場合（decode_with_disfluencyの返り値）
            if isinstance(hyp, dict):
                yseq = hyp["yseq"]
                disfluency_scores = hyp.get("disfluency_scores", None)
                score = hyp.get("score", None)
            else:
                yseq = hyp.yseq
                disfluency_scores = getattr(hyp, "disfluency_scores", None)
                score = getattr(hyp, "score", None)

            last_pos = None if self.asr_model.use_transducer_decoder else -1
            if isinstance(yseq, list):
                token_int = yseq[1:last_pos]
            else:
                token_int = yseq[1:last_pos].tolist()
            token_int = list(filter(lambda x: x != 0, token_int))
            token = self.converter.ids2tokens(token_int)
            text = self.tokenizer.tokens2text(token) if self.tokenizer is not None else None

            # Extract disfluency predictions if available
            disfluency_predictions = None
            if self.use_disfluency_detection:
                # statesフィールドから取得
                disfluency_scores = None
                if hasattr(hyp, "states") and isinstance(hyp.states, dict):
                    disfluency_scores = hyp.states.get("disfluency_scores", None)
                if disfluency_scores is not None:
                    if isinstance(disfluency_scores, list):
                        disfluency_predictions = disfluency_scores[:len(token_int)]
                    else:
                        disfluency_predictions = [
                            int(torch.argmax(torch.tensor(score)).item())
                            for score in disfluency_scores
                        ]
                        disfluency_predictions = disfluency_predictions[:len(token_int)]
                    if len(disfluency_predictions) < len(token_int):
                        disfluency_predictions.extend([0] * (len(token_int) - len(disfluency_predictions)))
            results.append((text, token, token_int, hyp, disfluency_predictions))
    ...
    def from_pretrained(
        ...
        **kwargs: Optional[Any],
        ...
        """Build DisfluencySpeech2Text instance from the pretrained model.
        ...
        Returns:
            DisfluencySpeech2Text: DisfluencySpeech2Text instance.
        ...
        return DisfluencySpeech2Text(**kwargs)
...
def inference(
    ...
    # Disfluency-specific parameters
    disfluency_weight: float,
    disfluency_classes: int,
    output_disfluency: bool,
):
    if batch_size > 1:
    ...
    speech2text = DisfluencySpeech2Text.from_pretrained(
        model_tag=model_tag,
        **speech2text_kwargs,
    )

    # 3. Build data-iterator
    loader = DisfluencyASRTask.build_streaming_iterator(
        data_path_and_name_and_type,
        dtype=dtype,
        batch_size=batch_size,
        key_file=key_file,
        num_workers=num_workers,
        preprocess_fn=DisfluencyASRTask.build_preprocess_fn(speech2text.asr_train_args, False),
        collate_fn=DisfluencyASRTask.build_collate_fn(speech2text.asr_train_args, False),
    ...
    with DatadirWriter(output_dir) as writer:
        for keys, batch in loader:
            ...
            # N-best list of (text, token, token_int, hyp_object, disfluency_predictions)
            # disfluency_predictions: 0=fluent, 1=filler, 2=reparandum, 3=interjection
            try:
                results = speech2text(**batch)
            except TooShortUttError as e:
                logging.warning(f"Utterance {keys} {e}")
                hyp = Hypothesis(score=0.0, scores={}, states={}, yseq=[])
                results = [[" ", ["<space>"], [2], hyp, None]] * nbest
                if enh_s2t_task:
            ...
            if enh_s2t_task or multi_asr:
                ...
                for spk, ret in enumerate(results, 1):
                    for n, (text, token, token_int, hyp, disfluency_preds) in zip(
                        range(1, nbest + 1), ret
                    ):
                        ...
                        # Write disfluency predictions
                        if disfluency_preds is not None:
                            ibest_writer[f"disfluency_spk{spk}"][key] = " ".join(
                                map(str, disfluency_preds)
                            )
                ...
                for n, (text, token, token_int, hyp, disfluency_preds) in zip(
                    ...
                    # Write disfluency predictions
                    if disfluency_preds is not None:
                        ibest_writer["disfluency"][key] = " ".join(
                            map(str, disfluency_preds)
                        )
...
def get_parser():
    parser = config_argparse.ArgumentParser(
        description="Disfluency ASR Decoding",
    ...
    # Disfluency-specific arguments
    group = parser.add_argument_group("Disfluency detection related")
    group.add_argument(
        "--disfluency_weight",
        type=float,
        default=1.0,
        help="Weight for disfluency loss in joint training",
    )
    group.add_argument(
        "--disfluency_classes",
        type=int,
        default=4,
        help="Number of disfluency classes (default: 4 - 0:fluent, 1:filler, 2:reparandum, 3:interjection)",
    )
    group.add_argument(
    "--output_disfluency",
    type=str2bool,
    default=False,
    help="If true, output disfluency predictions in the result.",
    )
